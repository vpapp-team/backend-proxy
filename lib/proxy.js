...
const handleRequest = (req, resp) => {
  ...
  if (req.headers.host === `${CONFIG.general.registration.hostname}:${CONFIG.general.registration.port}` &&
  ...
  } else if (CONFIG.endpoints.has(req.headers.host.split(':')[0])) {
    const mappingRule = CONFIG.endpoints.get(req.headers.host.split(':')[0]);
