...
for (const port of CFG.general.httpsPorts) {
  HTTPS_SERVERS.push(HTTPS.createServer(CFG.general.SECURE_CONTEXT.getCredentials(), requestHandled).listen(port));
}
CFG.general.SECURE_CONTEXT.on('CHANGE', () => {
  ...
  for(let a = 0 ; a < HTTPS_SERVERS.length ; a++) {
    HTTPS_SERVERS[a].close(() => {
      HTTPS_SERVERS[a] = HTTPS.createServer(CFG.general.SECURE_CONTEXT.getCredentials(), requestHandled)
