...
const handleRequest = (req, resp) => {
  ...
  if (req.headers.host === `${CFG.general.registration.hostname}${CFG.general.broadcast.port}` &&
    req.method === CFG.general.registration.method &&
    req.url === CFG.general.registration.url
  ) {
    handleRegister(req, resp);
  } else if (CFG.general.broadcast &&
    req.headers.host === `${CFG.general.broadcast.hostname}${CFG.general.broadcast.port}` &&
    req.method === CFG.general.broadcast.method &&
    ...
    // TODO: check whether this works with handlingServer.https = false
    // TODO: add header.uuid = req.handlingServer.uuid ??
  ...
  return UTIL.getBody(req).then(reqBody => {
    ...
    return (newServer.https ? HTTPS : HTTP).get({
      ...
      for (const ep of newServer.ep) {
        const cfg_ep = CFG.endpoints.get(ep);
